...
All benchmarks run on an inputs of size $2^{24}$, which is a typical number of pixels for an input image to a Halide program. We run the benchmarks on 8 cores of a Xeon E5-2690. In each case we use the same Halide \emph{algorithm} code, and compare the performance attainable with \code{rfactor} to the performance attainable without it. Table~\ref{tab:table} shows the results.
...
\multicolumn{1}{C{10cm}}{Tuple Size} & \multicolumn{1}{C{3cm}}{Size}\\
\midrule
Single-element (with constant, no \code{select}, max of 8 leaves) & 3749 \\
Single-dimensional (with constant, \code{select} only, max of 3 leaves for conditional, 4 leaves for the true/false) & 179 \\
Two-element (with constant, no \code{select}, max of 7 leaves) & 250 \\
\bottomrule
\end{tabular}
\end{center}
\label{default}
