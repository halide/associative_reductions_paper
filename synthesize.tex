...
We terminate our enumeration at trees with 8 leaves. We generate separate tables for each of Halide's scalar types, as some operations are associative in one type by not another. This takes 1.5 days and generates around 10,000 signed elementary operators to match against per type. Many of the operators are equivalent to simpler operators, but are written a more complex way. As we wish to catch all ways in which a programmer might write a reduction, we include these in the table. The first 10 elementary associative operators for signed 32-bit integers are shown in Listing~\ref{lst:top10}.
