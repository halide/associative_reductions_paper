...
int main(int argc, char **argv) {
    vector<vector<Expr>> leave_start_expr(2);
    leave_start_expr[0].reserve(32);
    leave_start_expr[1].reserve(32);
    ...
    while (true) {
        uint64_t index = 17950;
        Expr e0 = generate_expr(0, index, leaves[0], fails[0], leave_start_expr[0]);
        ...
        //Halide::Expr expr = (kXVars[1] * kYVars[1]) - (kXVars[0] * kYVars[0]);
        ...
        while (true) {
            Expr e1 = generate_expr(1, indices[1], leaves[1], fails[1], leave_start_expr[1]);
        ...
        return 0;
        leave_start_expr[1] = vector<Expr>();
