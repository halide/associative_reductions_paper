...
bool generate_expr(int index, uint64_t leaves, uint64_t i, set<Expr> &seen, Expr &e_cond, Expr &e_true, Expr &e_false) {
    string shift = "";
...
bool fast_check_associativity(const vector<Expr> &cond_eqs, const vector<Expr> &true_eqs,
                              const vector<Expr> &false_eqs, const set<AssociativeTuple> &associative_sets) {
    if (associative_sets.find(AssociativeTuple(cond_eqs)) != associative_sets.end()) {
        return false;
    }
        ...
        for (uint64_t leaves0 = leaves_start; leaves0 <= leaves_end; ++leaves0) {
            ...
            for (auto it0 = i0_range.begin(); it0 != i0_range.end(); it0++){
                for (uint64_t i0 = it0->lower(); i0 <= it0->upper(); ++i0) {
                    ...
                    Expr e0_cond, e0_true, e0_false;
                    bool skip_e0 = generate_expr(0, leaves0, i0, seen0[leaves0], e0_cond, e0_true, e0_false);
